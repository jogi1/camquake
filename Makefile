...
_E = @
CC = gcc -g
MGWCC = /usr/i586-mingw32msvc/bin/gcc
...
ifeq ($(ARCH),x86)
	ARCH_CFLAGS = -march=i686 -mtune=i686 -mmmx -Did386
endif
ifeq ($(ARCH),ppc)
...
MGW_TARGET = $(TYPE)-$(ARCH)/camquake.exe
QUAKE_DIR="/opt/quake/"

################

C_BUILD = $(_E)$(CC) -c -o $@ $(_CFLAGS) $<
MGW_BUILD = $(_E)$(MGWCC) -c -o $@ $(_CFLAGS) $<
...
$(GLX_DIR) $(X11_DIR) $(SVGA_DIR) $(MAC_DIR) $(MGW_DIR):
	$(MKDIR)
...
GLX_LDFLAGS = $(LDFLAGS) -lGL -lXxf86dga -lXxf86vm -lbz2 -lz
...
MGW_C_OBJS = $(addprefix $(MGW_DIR)/, $(addsuffix .o, $(MGW_C_FILES)))
MGW_CFLAGS = $(CFLAGS) $(GLCFLAGS) -D_MINGW32 -D_WIN32 -DWIN32 -I/usr/i586-mingw32msvc/include/
MGW_LDFLAGS = -g 

mgw: _DIR = $(MGW_DIR)
mgw: _OBJS = $(MGW_C_OBJS)
mgw: _LDFLAGS = $(MGW_LDFLAGS)
mgw: _CFLAGS = $(MGW_CFLAGS)
mgw: $(MGW_TARGET)

$(MGW_TARGET): $(MGW_DIR) $(MGW_C_OBJS) $(MGW_S_OBJS)
	@echo [LINK] $@
	$(BUILD)

$(MGW_C_OBJS): $(MGW_DIR)/%.o: %.c
	@echo [MGWCC] $<
	$(C_BUILD)
...
clean:
	@echo [CLEAN]
	@-rm -rf $(GLX_DIR) $(X11_DIR) $(SVGA_DIR) $(MAC_DIR) $(MGW_DIR)
