...
cvar_t	gl_ignore_max_size			= {"gl_ignore_max_size", "none"};
cvar_t	gl_ignore_max_size_size		= {"gl_ignore_max_size_size", "2048", 0, OnChange_gl_max_size};
...
static void ScaleDimensions1(int width, int height, int *scaled_width, int *scaled_height, int mode) 
{
	int maxsize, picmip;
	qbool scale = (mode & TEX_MIPMAP) && !(mode & TEX_NOSCALE);
	...
	maxsize = gl_ignore_max_size_size.value;
...
{
	int	i, scaled_width, scaled_height, crc = 0, old_max_size;
	qbool load_over_existing = false;
	gltexture_t *glt = NULL;
	char oldval[128];

	if (strlen(gl_ignore_max_size.string)>0)
	{
		if (strstr(identifier, gl_ignore_max_size.string))
		{
			Com_Printf("ignored %s\n", identifier);
			ScaleDimensions1(width, height, &scaled_width, &scaled_height, mode);
		}
		else
...
void GL_Texture_Init(void) 
{
	...
	Cvar_Register(&gl_ignore_max_size);
	Cvar_Register(&gl_ignore_max_size_size);
