...
void IN_MouseMove (usercmd_t *cmd) {
	extern cvar_t jogi_menu_system;
	static int old_mouse_x = 0, old_mouse_y = 0;
	extern double menu_mouse_x, menu_mouse_y;
	int mx, my;
#if DIRECTINPUT_VERSION	>= 0x0700
	int i, value;
	DIDEVICEOBJECTDATA od;
	...
	//
	if(key_dest == key_hudeditor || key_dest == key_menu || key_dest == key_demo_controls || key_dest == key_new_menu)
	{
		menu_mouse_x = old_mouse_x  = mouse_x = mx * cursor_sensitivity.value;
		menu_mouse_y = old_mouse_y = mouse_y = my * cursor_sensitivity.value;
